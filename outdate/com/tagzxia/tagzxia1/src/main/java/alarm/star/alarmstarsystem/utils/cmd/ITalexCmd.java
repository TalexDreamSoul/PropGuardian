package com.tagzxia.tagzxia1.src.main.java.alarm.star.alarmstarsystem.utils.cmd;
...
//            sendHelpMsg(sender);
        ...
        if (check(sender, method.getAnnotation(com.tagzxia.src.main.java.alarm.star.alarmstarsystem.utils.cmd.TalexCmd.class))) {
            try {
     ...
     * @return
     */
    private boolean check(CommandSender sender, com.tagzxia.src.main.java.alarm.star.alarmstarsystem.utils.cmd.TalexCmd cmd) {
        if (!sender.hasPermission(cmd.permission())) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', noPermission));
            return false;
        } else if (cmd.cmdSender() == com.tagzxia.src.main.java.alarm.star.alarmstarsystem.utils.cmd.TalexCmd.CmdSender.PLAYER && !(sender instanceof Player)) {
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', onlyPlayer));
            return false;
        } else if (cmd.cmdSender() == com.tagzxia.src.main.java.alarm.star.alarmstarsystem.utils.cmd.TalexCmd.CmdSender.CONSOLE && sender instanceof Player) {
    ...
    private Method getMethodByCommand(String subCmd) {
        ...
        for (Method method : methods) {
            com.tagzxia.src.main.java.alarm.star.alarmstarsystem.utils.cmd.TalexCmd cmd = method.getAnnotation(TalexCmd.class);
            if (cmd != null)
